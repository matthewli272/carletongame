...
//		boss = new Bosses("Boss", 100, 0,0);
	...
	public void draw() {
		...
		if (startMenu) {
			...
			textAlign(LEFT);
			textSize(12);
			text(level.getPlayer1().getName(), 25, 172);
			text(level.getPlayer2().getName(), 305, 172);
		...
		if (!startMenu) {
			...
			if (player1ChangeName) {
				if (((int) key >= 65 && (int) key <= 90) || ((int) key >= 97 && (int) key <= 122))
					level.getPlayer1().setName(level.getPlayer1().getName() + key);
				else if (key == 8)
					level.getPlayer1().setName(level.getPlayer1().getName().substring(0, level.getPlayer1().getName().length() - 1));
			} else if (player2ChangeName) {
				if (((int) key >= 65 && (int) key <= 90) || ((int) key >= 97 && (int) key <= 122))
					level.getPlayer2().setName(level.getPlayer2().getName() + key);
				else if (key == 8)
					level.getPlayer2().setName(level.getPlayer2().getName().substring(0, level.getPlayer2().getName().length() - 1));
			}
		}
	}
	 ...
	 * switch (e.getKeyCode()) { case KeyEvent.VK_W: if(!test.contains("w"))
	 * test.add("w"); break; } }
	 * 
	 * @Override public void keyReleased(KeyEvent e) { System.out.println("yes");
	 * switch (e.getKeyCode()) { case KeyEvent.VK_W: test.remove("w"); break; } }
	 * 
