...
public class DrawingSurface extends PApplet /*implements MouseListener, ActionListener, KeyListener*/ {
	...
	public void draw() { 
		...
		if(state == State.MENU){
			...
			if(player1movement.contains("w")) {
				level1.getPlayer1().setY((int) (level1.getPlayer1().getY() - cellHeight) );
			}else if(player1movement.contains("a"))
			{
				level1.getPlayer1().setX((int) (level1.getPlayer1().getX() - cellWidth) );
...
///*
			if(player2movement.contains("w")) {
				level1.getPlayer2().setY((int) (level1.getPlayer2().getY() - cellHeight) );
			}else if(player2movement.contains("a"))
			{
				level1.getPlayer2().setX((int) (level1.getPlayer2().getX() - cellWidth) );
		...
		if(state != State.MENU) {
			...
			if(player1ChangeName) {
				if (((int) key >= 65 && (int) key <= 90) || ((int) key >= 97 && (int) key <= 122))
					...
					level1.getPlayer1().setName(level1.getPlayer1().getName().substring(0,level1.getPlayer1().getName().length() - 1));
			} else if (player2ChangeName) {
				if (((int) key >= 65 && (int) key <= 90) || ((int) key >= 97 && (int) key <= 122))
					level1.getPlayer2().setName(level1.getPlayer2().getName() + key);
				else if(key == 8)
					level1.getPlayer2().setName(level1.getPlayer2().getName().substring(0,level1.getPlayer2().getName().length() - 1));
			}
		}
	}
